...
describe('Avti CLI control routing', () => {
  it('keeps product control commands out of the one-shot agent prompt path', () => {
    expect(resolveAvtiInvocation(['status'])).toEqual({
      mode: 'control',
      command: 'status',
      args: [],
    })
    expect(resolveAvtiInvocation(['models', 'deepseek-official'])).toEqual({
      mode: 'control',
      command: 'models',
      args: ['deepseek-official'],
    })
    expect(resolveAvtiInvocation(['model', 'deepseek-official', 'deepseek-v4-flash'])).toEqual({
      mode: 'control',
      command: 'model',
      args: ['deepseek-official', 'deepseek-v4-flash'],
    })
    expect(resolveAvtiInvocation(['sessions'])).toEqual({
      mode: 'control',
      command: 'sessions',
      args: [],
    })

...
export function verifyWindowsPortable(
  ...
  const portablePath = join(
    options.desktopRoot,
    'dist',
    `Avti-${options.version}-x64-Portable.zip`,
  )
  ...
  const executable = entries.find(entry => entry.entryName.replaceAll('\\', '/') === 'Avti.exe')
  if (executable === undefined) {
    throw new Error(`Windows portable archive is missing Avti.exe: ${portablePath}`)
  }
